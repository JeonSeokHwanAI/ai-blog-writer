# 블로그 글쓰기

인자: $ARGUMENTS

블로그 글을 작성하기 위한 마크다운 파일을 생성하고, 단계별로 글을 완성합니다.

## 사용법

- `/write` → 새 글 생성 (이전 키워드 검색 결과 활용)
- `/write 제목` → 지정한 제목으로 새 글 생성

---

## 실행 흐름

### 0단계: 페르소나 로딩 (필수 - 가장 먼저 실행)

⚠️ **글 작성 전 반드시 페르소나를 완전히 로딩합니다. 이 단계를 건너뛰면 안 됩니다.**

```
1. config/.active_persona.json 읽기 → 활성 페르소나 이름 확인
2. config/personas/{이름}.json 읽기 → 스타일 로드
3. ⚠️ context_file 필드 확인 → 있으면 반드시 해당 문서 읽기
4. docs/context/Naver Blog Writer.md 읽기
```

**로딩 완료 후 확인 출력:**

```
## 페르소나 로딩 완료

| 항목 | 값 |
|------|-----|
| 페르소나 | {이름} |
| 닉네임 | {profile.nickname} |
| 톤 | {style.tone} |
| 타겟 독자 | {context_file에서 추출} |
| 핵심 고민 | {context_file에서 추출} |
| 글 마무리 | {context_file에서 추출} |
```

**context_file이 있으면 반드시 읽어야 하는 항목:**
- 타겟 독자 (주요 독자층)
- 독자의 핵심 고민
- 콘텐츠 스타일 (글쓰기 톤, 활용 소재, 글의 구조)
- 글 마무리 공식
- 차별화 포인트

### 1단계: 컨텍스트 확인

**이전 대화에서 `/keyword`, `/bridge`, `/clip` 등을 실행했는지 확인합니다.**

실행한 경우 → 마지막 키워드/주제를 기억합니다.

### 2단계: 키워드 확인 질문

AskUserQuestion으로 질문:

```
이전에 검색한 키워드를 기준으로 블로그를 작성할까요?

키워드: {마지막 검색 키워드}
```

**옵션:**
- 예 → 해당 키워드 기반으로 진행
- 아니오 → 새로운 주제로 진행

### 3단계: 추가 내용 입력

텍스트로 질문 (AskUserQuestion 사용하지 않음):

```
추가로 작성할 내용이 있으면 입력하세요.
(없으면 엔터만 누르세요)
```

사용자 입력:
- 내용 입력 → **작성자 원본** 섹션에 저장
- 빈 줄/엔터 → 바로 파일 생성

### 3-1단계: 작성자 원본 사전 체크

사용자가 내용을 입력하면 **즉시** 다음을 체크하여 표시:

1. **금칙어 스캔**: `config/naver_forbidden_words.json`의 categories 전체에서 매칭
2. **글자 수 표시**: 공백/줄바꿈 제거 후 글자 수 계산

```
📝 작성자 원본 분석

📊 글자 수: {공백 제외}자
```

금칙어 발견 시 별도 테이블 추가:

```
⚠️ 금칙어 감지 ({N}개)

| # | 단어 | 위험도 | 카테고리 | 대체어 |
|---|------|--------|----------|--------|
| 1 | 완치 | 🔴 금지 | 의료·건강 | 호전, 개선 |
| 2 | 협찬 | 🟡 주의 | 저품질 유발 | 제공, 지원 |

💡 🔴 금지 단어는 본문 작성 시 자동으로 대체됩니다.
💡 🟡 주의 단어는 문맥에 따라 유지/변경됩니다.
```

미발견 시: `✅ 금칙어 없음` 표시

**이 체크는 사용자에게 정보 제공 목적이며, 원본 텍스트는 수정하지 않습니다.**
AI가 본문(5단계)을 작성할 때 금칙어를 자동 회피/대체합니다.

### 4단계: 파일 생성

`docs/blog/` 폴더에 마크다운 파일 생성:
- 파일명: `{키워드 또는 제목}_{YYYYMMDD}.md`

### 5단계: 본문 작성 (초안)

키워드와 사용자 입력을 바탕으로 **본문** 섹션에 초안 작성.

**이 단계에서는 추천 제목, Image Prompt, Tags를 작성하지 않습니다.**

⚠️ **초안 작성 시에도 아래 "필수 적용 규칙"을 반드시 따릅니다.**

```
## 본문 초안 작성 완료

[초안 내용 표시]

📊 글자 수: {공백 제외}자 / 목표: {min}~{max}자
   ⚠️ 부족 ({차이}자 추가 필요)  ← 범위 미달 시
   ⚠️ 초과 ({차이}자 줄이기 권장)  ← 범위 초과 시
   ✅ 적정 범위                   ← 범위 내

---
글을 완성할까요? (제목, 이미지 프롬프트, 태그 생성)
```

### 6단계: 완성 확인 질문

AskUserQuestion으로 질문:

```
완성형으로 적용하시겠습니까?

@docs/context/Naver Blog Writer.md
활성 페르소나: {현재 활성 페르소나 이름}

위 가이드와 활성 페르소나 스타일을 적용하여 글을 다듬고, 제목/이미지/태그를 생성합니다.
```

**옵션:**
- 예 → 7단계 진행
- 다시 작성 → 본문 재작성 (6단계 반복, 완성하지 않음)
- 아니오 → 초안 상태로 저장

### 7단계: 글 완성

**완성 시** 다음을 순서대로 실행:

1. ⚠️ **본문 Clean Text Mode 검증 및 재작성**
   - `docs/context/Naver Blog Writer.md` 읽고 규칙 확인
   - `config/.active_persona.json` → `config/personas/{이름}.json` 읽고 스타일 적용
   - 페르소나의 `context_file` 필드가 있으면 해당 문서도 참조
   - 볼드체(`**`)가 있으면 모두 제거
   - 마크다운 서식(`###`, `-`, `1.`)이 있으면 줄글로 변환
   - "여러분" → "당신"으로 변경
   - 시작 문구 확인 ("안녕하세요, {활성 페르소나 닉네임}입니다.")

2. ⚠️ **금칙어 + 누락 패턴 체크**
   - `config/naver_forbidden_words.json` 로드
   - **[A] 단어 매칭**: 본문에서 금칙어/주의어 매칭 검사 (categories 전체)
   - **[B] 패턴 체크**: `_omission_patterns` 섹션의 규칙 적용
     - 키워드 스터핑: 같은 단어 5회 이상 반복 → 🔴
     - 외부 링크 과다: http/https 링크 3개 이상 → 🟡
     - 제휴 링크: coupang, affiliate URL 패턴 → 🔴
   - 🔴 error (금지): 반드시 대체어로 자동 교체 / 패턴 수정
   - 🟡 warning (주의): 대체어 제안, 사용자 판단에 맡김

3. ⚠️ **글자 수 체크 및 조정**
   - 본문 섹션만 추출 (## 본문 ~ 다음 --- 사이)
   - 공백/줄바꿈 제거 후 글자 수 계산
   - 목표 범위: 페르소나의 `style.target_length` (없으면 질문 후 저장)
   - 범위 미달 시 → 본문 보강 후 재확인
   - 범위 초과 시 → 본문 축약 후 재확인
   - 조정 후 최종 글자 수 출력

4. ⚠️ **본문 분석 결과를 파일의 `## 본문` 섹션 끝에 기록** (본문 텍스트 뒤, `---` 구분선 앞)
   - 대화창 출력이 아닌 **파일에 직접 기록**
   - 형식:
     ```
     📊 본문 분석
     글자 수: {공백 제외}자 (공백 제외) / 목표: {min}~{max}자 ✅ 적정 범위

     ⚠️ 금칙어/누락 패턴 체크 결과

     [단어]
     | # | 단어 | 위험도 | 카테고리 | 대체어 | 판정 |
     |---|------|--------|----------|--------|------|
     | 1 | 완치 | 🔴 금지 | 의료·건강 | 호전, 개선 | 대체 완료 |
     | 2 | 1위 | 🟡 주의 | 과장·허위광고 | (출처 명시 필요) | 출처 명시됨 → 유지 |

     [패턴]
     | # | 패턴 | 위험도 | 내용 |
     |---|------|--------|------|
     | 1 | 키워드 스터핑 | ✅ | 해당 없음 |
     | 2 | 외부 링크 | ✅ | 0개 |
     | 3 | 제휴 링크 | ✅ | 0개 |
     ```
   - 금칙어 미발견 시: `✅ 금칙어 없음` 한 줄로 표시
   - [단어] 테이블에 **판정** 컬럼 필수 (유지/대체 사유 기재)

5. **추천 제목** 5가지 생성 (후크형, 일반형, SEO, 궁금증, 숫자형)
   - ⚠️ **제목 공식: 검색 키워드 + 후크** (키워드가 앞, 후크가 뒤)
   - 모든 제목에 반드시 **검색 의도 키워드**를 포함 (사람들이 실제로 검색하는 단어)
   - 키워드 없이 후크만 있는 제목은 금지 (예: ❌ "못한다고요? 메모장으로 증명했습니다")
   - 좋은 예: ✅ "AI 업무 자동화, 못한다고요? 메모장 하나로 증명했습니다"
   - 이전 `/keyword`, `/trend` 결과가 있으면 골든키워드 우선 활용

6. **Image Prompt 4개** 본문 흐름에 맞춰 생성
   - 반드시 **16:9 비율** 명시 (Aspect ratio 16:9, horizontal composition)
   - 각 이미지에 **위치**, **스타일**, **비율** 기재
   - 본문의 주요 섹션별로 1개씩 배치

7. **Tags** 추천 태그 10개 생성

8. **카테고리 추천** (캐시가 있을 때만)
   - 활성 페르소나의 `blog_id` 확인 → `output/{blog_id}/{blog_id}_categories.json` 로드
   - 캐시 파일이 없으면 이 단계 건너뜀 (frontmatter `category`는 빈 값 유지)
   - 본문 내용과 카테고리 목록을 비교하여 적합한 카테고리 2개 추천
   - 1순위 카테고리를 frontmatter의 `category` 필드에 기입
   - 출력:
     ```
     ## 추천 카테고리

     | 순위 | 카테고리 | 이유 |
     |------|----------|------|
     | 1 | {카테고리명} | {추천 이유} |
     | 2 | {카테고리명} | {대안 이유} |
     ```

9. 상태를 `complete`로 변경

---

## 파일 템플릿

```markdown
---
title: "{제목}"
created_at: "{YYYY-MM-DD HH:MM}"
status: "draft"  # draft → complete
keyword: "{키워드}"
category: ""
persona: "{활성 페르소나 이름}"
persona_file: "{config/personas/이름.json 경로}"
---

# {제목}

---

## 작성자 원본

> 사용자가 직접 작성한 영역 (AI 영역 아님)

{사용자 입력 내용}

---

## 본문

{AI가 작성한 블로그 글}

{📊 본문 분석 — 7단계 완성 시 여기에 글자 수 + 금칙어/패턴 체크 결과 기록}

---

## 추천 제목

| 유형 | 제목 |
|------|------|
| 후크형 | {완성 시 작성} |
| 일반형 | {완성 시 작성} |
| SEO 최적화 | {완성 시 작성} |
| 궁금증 유발 | {완성 시 작성} |
| 숫자형 | {완성 시 작성} |

---

## Image Prompt (4개)

> 완성 시 본문 흐름에 맞춰 4개 작성

### 1. {섹션명}
**위치**: {본문 내 위치}
**스타일**: {일러스트 스타일}
**비율**: 16:9
```
{프롬프트 내용}. Aspect ratio 16:9, horizontal composition.
```

### 2. {섹션명}
**위치**: {본문 내 위치}
**스타일**: {일러스트 스타일}
**비율**: 16:9
```
{프롬프트 내용}. Aspect ratio 16:9, horizontal composition.
```

### 3. {섹션명}
**위치**: {본문 내 위치}
**스타일**: {일러스트 스타일}
**비율**: 16:9
```
{프롬프트 내용}. Aspect ratio 16:9, horizontal composition.
```

### 4. {섹션명}
**위치**: {본문 내 위치}
**스타일**: {일러스트 스타일}
**비율**: 16:9
```
{프롬프트 내용}. Aspect ratio 16:9, horizontal composition.
```

---

## Tags

> 완성 시 작성

```
#태그1 #태그2 #태그3
```

---

## 메모

```

---

## 필수 적용 규칙 (Clean Text Mode)

⚠️ **본문 작성 시 반드시 아래 규칙을 적용합니다. 초안/완성 모두 동일하게 적용.**

### 1. Naver Blog Writer 규칙 (`docs/context/Naver Blog Writer.md`)

| 규칙 | 설명 |
|------|------|
| 볼드체 금지 | `**텍스트**` 절대 사용 금지 |
| 마크다운 서식 금지 | `###` 제목, `-` 리스트, `1.` 숫자목록 금지 |
| 줄글 유지 | 모든 내용은 자연스러운 문장으로 작성 |
| AI 멘트 금지 | "작성하겠습니다", "분석해보면" 등 금지 |
| 1:1 대화체 | "여러분" 금지 → "당신"으로 작성 |

### 2. 활성 페르소나 적용 (필수)

⚠️ **0단계에서 로딩한 페르소나를 글 전체에 적용합니다.**

**JSON에서 적용:**

| 항목 | 참조 필드 |
|------|-----------|
| 시작 문구 | "안녕하세요, {profile.nickname}입니다." |
| 톤앤매너 | style.tone, style.formality |
| 글쓰기 패턴 | style.writing_patterns |
| 자주 쓰는 표현 | style.favorite_expressions |
| 글 구조 | style.content_structure |

**context_file에서 적용 (있으면 필수):**

| 항목 | 적용 방법 |
|------|-----------|
| 타겟 독자 | 그 독자의 눈높이에 맞춰 작성 |
| 핵심 고민 | 도입부에서 공감, 본문에서 해결 |
| 콘텐츠 스타일 | 글쓰기 톤, 소재, 구조 반영 |
| 글 마무리 공식 | 마무리에 "바로 해볼 수 있는 한 가지 액션" 제시 |
| 차별화 포인트 | 글에 자연스럽게 녹여내기 |

**활성 페르소나가 없으면:** 페르소나 없이 Naver Blog Writer 규칙만 적용.

**우선순위:** context_file > 페르소나 JSON > Naver Blog Writer

---

## 글자 수 체크

**측정 방법:**
- 본문 섹션만 대상 (`## 본문` ~ 다음 `---` 사이)
- 공백과 줄바꿈을 모두 제거한 순수 글자 수
- Python: `len(body.replace(' ', '').replace('\n', ''))`

**목표 범위 결정:**

1. 페르소나 JSON의 `style.target_length` 필드 확인

2. **target_length가 있을 때** → 현재 설정을 알려주고 변경 여부 확인:
   ```
   현재 페르소나({이름})의 목표 글자 수: {min}~{max}자 (공백 제외)
   이 글에 다른 글자 수를 적용하시겠습니까?
   ```
   옵션:
   - 현재 설정 유지 ({min}~{max}자) (Recommended)
   - 짧은 글 (1,000 ~ 1,500자) — 가벼운 팁, 일상 공유
   - 긴 글 (2,000 ~ 3,000자) — 심층 분석, 튜토리얼
   - 직접 입력 — min~max 직접 지정

   → "현재 설정 유지" 선택 시 바로 진행
   → 다른 옵션 선택 시 **이 글에만** 적용 (페르소나 JSON은 수정하지 않음)

3. **target_length가 없을 때** → 기본 설정 질문:
   ```
   목표 글자 수 범위를 설정해주세요. (공백 제외 기준)
   ```
   옵션:
   - 짧은 글 (1,000 ~ 1,500자) — 가벼운 팁, 일상 공유
   - 보통 글 (1,500 ~ 2,000자) — 경험 공유, 리뷰 (Recommended)
   - 긴 글 (2,000 ~ 3,000자) — 심층 분석, 튜토리얼
   - 직접 입력 — min~max 직접 지정

   → 선택 후 페르소나 JSON에 `style.target_length`로 자동 저장
   → 다음 `/write`부터는 2번 흐름으로 진행

**표시 시점:**
1. 5단계 (초안 완료) → 글자 수 + 범위 판정
2. 7단계 (완성) → 2번 글자 수 체크에서 조정 후 최종 글자 수

**범위 이탈 시 대응:**

| 상태 | 조건 | 대응 |
|------|------|------|
| 부족 | < min | 본문 보강 (경험 묘사, 공감 문장, 구체적 예시 추가) |
| 적정 | min ~ max | 그대로 진행 |
| 초과 | > max | 중복 표현 제거, 불필요한 문장 축약 |

---

## 중요 규칙

### 완성 조건
- **추천 제목, Image Prompt, Tags가 모두 작성되어야 완성**
- 사용자가 "완성해줘"라고 할 때만 완성 단계 진행
- "다시 작성해줘"는 본문만 수정, 완성하지 않음

### 재작성 요청 시
- 본문만 다시 작성
- 추천 제목, Image Prompt, Tags는 건드리지 않음
- "글 완성할까요?" 다시 질문

### 컨텍스트 참조
완성 단계에서 참조할 파일:
- `docs/context/Naver Blog Writer.md`: 네이버 블로그 작성 가이드
- `config/.active_persona.json` → `config/personas/{이름}.json`: 활성 페르소나
- `config/naver_forbidden_words.json`: 금칙어 목록
- 페르소나의 `context_file` 필드가 있으면 해당 문서도 참조

---

## 저장 위치

`docs/blog/{키워드 또는 제목}_{YYYYMMDD}.md`

## 참조 파일

- `config/.active_persona.json`: 활성 페르소나 정보
- `config/personas/`: 페르소나별 JSON 파일
- `config/naver_forbidden_words.json`: 금칙어/주의어 목록
- `docs/context/Naver Blog Writer.md`: 블로그 작성 가이드
- `docs/blog/`: 블로그 글 저장 폴더
