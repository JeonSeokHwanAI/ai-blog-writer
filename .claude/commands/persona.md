...
- `import` → 기존 페르소나 파일 목록 표시
- `import <이름|경로>` → 기존 파일에서 페르소나 가져오기
...
### 4. 기존 파일에서 가져오기 (`import`)

이미 작성된 페르소나 마크다운 파일을 JSON 페르소나로 변환합니다.

#### 4-1. 파일 목록 보기 (`/persona import`)

`docs/context/` 폴더에서 페르소나 관련 파일을 검색하여 목록 표시:
...
| # | 파일명 | 경로 |
|---|--------|------|
| 1 | 셀슈머 | docs/context/페르소나 - 셀슈머.md |
| 2 | AI블로그작가 | docs/context/페르소나 - AI블로그작가.md |

---
💡 `/persona import 셀슈머` - 해당 페르소나 가져오기
💡 `/persona import 1` - 번호로 가져오기
```

**검색 패턴:**
- `docs/context/페르소나 - *.md`
- `docs/context/페르소나*.md`
- `docs/context/*페르소나*.md`

#### 4-2. 파일 가져오기 (`/persona import <이름|경로|번호>`)

**인자 유형:**
- `<이름>` → `docs/context/페르소나 - <이름>.md` 파일 검색
- `<번호>` → import 목록의 해당 번호 파일
- `<경로>` → 직접 파일 경로 지정
...
2. **내용 분석 및 변환**
   ...
   Claude가 파일 내용을 분석하여 JSON 구조로 변환:
...
3. **JSON 생성**
   ...
   `config/personas/<이름>.json` 저장 (source: "import")

   ```json
   {
     "name": "셀슈머",
     "source": "import",
     "import_file": "docs/context/페르소나 - 셀슈머.md",
...
4. **활성화 여부 질문**
   ...
   |------|------|
   | 이름 | 셀슈머 |
   | 원본 파일 | docs/context/페르소나 - 셀슈머.md |
   | 저장 위치 | config/personas/셀슈머.json |

   이 페르소나로 빙의할까요? (현재: AI블로그작가)
   ```
...
|---------------|-----------|
| 제목 (# 페르소나명) | name |
| 닉네임/별명 | profile.nickname |
| 소개/한 줄 소개 | profile.intro |
| 주제/분야 | profile.topics, content_themes |
| 톤/어조/스타일 | style.tone, style.formality |
| 자주 쓰는 표현 | style.favorite_expressions |
| 글 구조 | style.content_structure |
| 독자/타겟 | (별도 필드 또는 persona에 포함) |
| 페르소나 설명 | persona |
| 샘플 문장 | voice_samples |
